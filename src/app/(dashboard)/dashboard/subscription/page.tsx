  ...
  const runReconcile = useCallback(async () => {
    ...
    try {
      ...
      if (result.reconciled && result.tier !== 'FREE') {
        ...
        await fetchData();
        toast.success('Subscription aktif!', {
          description: `Plan kamu sekarang ${result.tier}.`,
        });
      } else {
        ...
        toast.error('Verifikasi gagal', {
          description:
            'Silakan coba lagi atau hubungi support jika pembayaran sudah terpotong.',
        });
      }
    } catch (err) {
      setVerifyState('failed');
      toast.error('Reconcile gagal', { description: getErrorMessage(err) });
    }
  }, [fetchData]);
        ...
        if (result.status === 'completed') {
          ...
          toast.success('Payment successful!', {
            description: result.subscription
              ? `Plan kamu sekarang ${result.subscription.tier}.`
              : 'Subscription kamu aktif.',
          });
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      {verifyState === 'verifying' && (
        <Alert>
          ...
          <AlertDescription>
            <strong>Memproses pembayaran...</strong> Tunggu beberapa detik sampai
            subscription kamu aktif. Jangan refresh halaman.
          </AlertDescription>
        </Alert>
      )}
      ...
      {verifyState === 'reconciling' && (
        <Alert>
          <Loader2 className="h-4 w-4 animate-spin" />
          <AlertDescription>
            <strong>Mengecek status pembayaran ke Stripe...</strong> Proses ini
      ...
      {verifyState === 'completed' && (
        <Alert className="border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/30">
          ...
          <AlertDescription className="text-emerald-800 dark:text-emerald-300">
            <strong>Subscription aktif!</strong> Plan kamu sudah di-upgrade.
            Selamat menikmati fitur baru! 🎉
          </AlertDescription>
        </Alert>
      )}
      ...
      {verifyState === 'failed' && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="space-y-2">
            <div>
              <strong>Verifikasi pembayaran gagal.</strong> Jika kartu kamu sudah
              terpotong, pembayaran akan diverifikasi otomatis dalam beberapa
              menit. Kamu bisa coba lagi atau hubungi support di{' '}
