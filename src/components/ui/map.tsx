...
function MapTileLayer({
  ...
  // [DESIGN.md AUDIT — Agu 2026] Default basemap, riwayat singkat:
  // CARTO Positron → wajib API key sekarang (carto.com/basemaps/apikey).
  // Stamen Toner (Stadia) → B&W literal tapi kontras tinggi & padat di
  // semua fitur (didesain sebagai backdrop untuk overlay data warna-warni,
  // bukan untuk tampilan minimalis) — kepadatan jalan/gang di area urban
  // Indonesia bikin label numpuk dan keliatan berantakan di zoom dekat.
  // Diganti ke Alidade Smooth (Stadia) karena ini yang dirancang untuk
  // "clean": skema warna lembut/muted dan sengaja lebih sedikit titik
  // minat, supaya marker/data yang jadi fokus, bukan basemap-nya. Ini cuma
  // peta formalitas kontak, jadi kesederhanaan lebih penting daripada
  // presisi gaya hitam-putih literal.
  //
  // Tetap Stadia Maps (bukan ganti provider lagi) karena sudah
  // terverifikasi: raster PNG biasa (bukan vector/MapLibre — dihindari
  // sengaja karena provider vector sebelumnya, OpenFreeMap, pernah bikin
  // bug stale-chunk HMR + tabrakan hydration mismatch di Next.js dev
  // server), dan akses localhost/127.0.0.1 tidak butuh API key sama sekali
  // (docs.stadiamaps.com/authentication). Free tier non-commercial berlaku
  // sampai 200.000 tile/bulan.
  //
  // PENTING kalau nanti deploy ke domain publik (fibidy.com dll): akses
  // localhost yang "tanpa key" ini TIDAK otomatis berlaku di production.
  // Perlu daftar akun gratis di Stadia Maps lalu tambahkan domain lewat
  // domain-based authentication di dashboard mereka, atau pasang API key —
  // supaya tidak kena rate-limit/block yang sama seperti kasus CARTO.
  const DEFAULT_URL =
  ...
  const resolvedAttribution =
    resolvedTheme === "dark" && darkAttribution
      ? darkAttribution
      : (attribution ??
        '&copy; <a href="https://www.stadiamaps.com/" target="_blank" rel="noopener">Stadia Maps</a>, &copy; <a href="http://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a> contributors')
