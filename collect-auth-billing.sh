...
# ================================================================
# CLIENT — AUTH + BILLING/SUBSCRIPTION COVERAGE COLLECTOR
# [VERCEL VIBES — Aug 2026]
#
# Collect semua file yang terlibat dalam auth flow DAN subscription
# billing flow (Tripay + LemonSqueezy):
#
# ── AUTH ─────────────────────────────────────────────────────────
...
#  10.  App route — dashboard/subscription page
#  11.  API layer — subscription.ts (semua call ke backend)
#  12.  Hook — use-subscription-plan.ts
#  13.  UI — upgrade-modal.tsx (dipanggil dari mana saja di dashboard)
#  14.  Types — tenant.ts (subscriptionTier, subscriptionStatus)
#  15.  Config — features.ts (flag subscription)
#  16.  Auth layer yang dipakai billing (client.ts, auth-store, use-auth)
#
# ── QUALITY CHECKS ───────────────────────────────────────────────
# AUTH:
...
#
# BILLING:
#   H.  subscription.ts ada endpoint Tripay (tripay)
#   I.  subscription.ts ada endpoint LemonSqueezy (ls / lemon)
#   J.  use-subscription-plan.ts handle status PENDING / PAID / EXPIRED
#   K.  upgrade-modal.tsx ada cabang Tripay (qr / qris / tripay)
#   L.  upgrade-modal.tsx ada cabang LS / LemonSqueezy
#   M.  tenant.ts ada field subscriptionTier + subscriptionStatus
#   N.  subscription page.tsx TIDAK ada 'use client' (server component)
#   O.  client.ts ada Authorization header (Bearer token)
#
# Run dari: root direktori client (tempat src/ ada)
#   bash fibidy-billing-auth-collect.sh
# ================================================================
...
check_proxy_auth_gate() {
    local output=$1
    section_header "CHECK A — proxy.ts: auth gate" "$output"
...
# ================================================================
# CHECK H — subscription.ts ada endpoint Tripay
# ================================================================
check_subscription_has_tripay() {
    local output=$1
    section_header "CHECK H — subscription.ts: ada endpoint Tripay" "$output"

    local file="$SRC_DIR/lib/api/subscription.ts"
    if [ ! -f "$file" ]; then
        check_missing "subscription.ts tidak ditemukan!" "$output"
        return
    fi

    if grep -qiE "tripay" "$file" 2>/dev/null; then
        check_pass "subscription.ts ada referensi endpoint Tripay" "$output"
    else
        check_fail "subscription.ts TIDAK ada endpoint Tripay — belum di-wire!" "$output"
    fi

    if grep -qiE "checkout|createCheckout|create.?transaction" "$file" 2>/dev/null; then
        check_pass "subscription.ts ada fungsi checkout/create transaction" "$output"
    else
        check_warn "subscription.ts tidak ada fungsi checkout — perlu dicek" "$output"
    fi
...
# ================================================================
# CHECK I — subscription.ts ada endpoint LemonSqueezy
# ================================================================
check_subscription_has_ls() {
    local output=$1
    section_header "CHECK I — subscription.ts: ada endpoint LemonSqueezy" "$output"
    ...
    if grep -qiE "lemon|lemonsqueezy|ls" "$file" 2>/dev/null; then
        check_pass "subscription.ts ada referensi endpoint LemonSqueezy" "$output"
    else
        check_warn "subscription.ts tidak ada referensi LS — mungkin belum diimplementasi atau sudah dihapus" "$output"
    fi
...
# ================================================================
# CHECK J — use-subscription-plan.ts handle status Tripay
# ================================================================
check_hook_handles_tripay_status() {
    local output=$1
    section_header "CHECK J — use-subscription-plan.ts: handle status Tripay" "$output"

    local file="$SRC_DIR/hooks/dashboard/use-subscription-plan.ts"
    if [ ! -f "$file" ]; then
        check_missing "use-subscription-plan.ts tidak ditemukan!" "$output"
        return
    fi

    for status in "PENDING" "PAID" "EXPIRED"; do
        if grep -q "$status" "$file" 2>/dev/null; then
            check_pass "use-subscription-plan.ts handle status $status" "$output"
        else
            check_warn "use-subscription-plan.ts tidak handle status $status — perlu dicek" "$output"
        fi
    done
...
# ================================================================
# CHECK K — upgrade-modal.tsx ada cabang Tripay
# ================================================================
check_upgrade_modal_has_tripay() {
    local output=$1
    section_header "CHECK K — upgrade-modal.tsx: ada cabang Tripay (QR/QRIS)" "$output"

    local file="$SRC_DIR/components/dashboard/shared/upgrade-modal.tsx"
    if [ ! -f "$file" ]; then
        check_missing "upgrade-modal.tsx tidak ditemukan!" "$output"
        return
    fi

    if grep -qiE "tripay|qris|qr" "$file" 2>/dev/null; then
        check_pass "upgrade-modal.tsx ada cabang Tripay/QRIS" "$output"
    else
        check_fail "upgrade-modal.tsx TIDAK ada cabang Tripay — belum di-wire!" "$output"
    fi
...
# ================================================================
# CHECK L — upgrade-modal.tsx ada cabang LemonSqueezy
# ================================================================
check_upgrade_modal_has_ls() {
    local output=$1
    section_header "CHECK L — upgrade-modal.tsx: ada cabang LemonSqueezy" "$output"
    ...
    if grep -qiE "lemon|lemonsqueezy|\bls\b" "$file" 2>/dev/null; then
        check_pass "upgrade-modal.tsx ada cabang LemonSqueezy" "$output"
    else
        check_warn "upgrade-modal.tsx tidak ada cabang LS — mungkin belum diimplementasi" "$output"
    fi
...
# ================================================================
# CHECK M — tenant.ts ada field subscriptionTier + subscriptionStatus
# ================================================================
check_tenant_types_subscription() {
    local output=$1
    section_header "CHECK M — tenant.ts: ada field subscription" "$output"

    local file="$SRC_DIR/types/tenant.ts"
    if [ ! -f "$file" ]; then
        check_missing "tenant.ts tidak ditemukan!" "$output"
        return
    fi

    for field in "subscriptionTier" "subscriptionStatus" "subscriptionPeriodEnd"; do
        if grep -q "$field" "$file" 2>/dev/null; then
            check_pass "tenant.ts ada field $field" "$output"
        else
            check_warn "tenant.ts tidak ada field $field — perlu dicek" "$output"
        fi
    done
...
# ================================================================
# CHECK N — subscription page.tsx TIDAK ada 'use client'
# ================================================================
check_subscription_page_no_use_client() {
    local output=$1
    section_header "CHECK N — subscription/page.tsx: tidak boleh 'use client'" "$output"

    local file="$SRC_DIR/app/[locale]/(dashboard)/dashboard/subscription/page.tsx"
    if [ ! -f "$file" ]; then
        check_missing "subscription/page.tsx tidak ditemukan!" "$output"
        return
    fi

    if grep -q "'use client'" "$file" 2>/dev/null; then
        check_fail "subscription/page.tsx ada 'use client' — harus server component!" "$output"
    else
        check_pass "subscription/page.tsx bersih dari 'use client' (server component)" "$output"
    fi
...
# ================================================================
# CHECK O — client.ts ada Authorization Bearer header
# ================================================================
check_client_has_auth_header() {
    local output=$1
    section_header "CHECK O — client.ts: ada Authorization Bearer header" "$output"

    local file="$SRC_DIR/lib/api/client.ts"
    if [ ! -f "$file" ]; then
        check_missing "client.ts tidak ditemukan!" "$output"
        return
    fi

    if grep -qiE "Authorization|Bearer" "$file" 2>/dev/null; then
        check_pass "client.ts ada Authorization/Bearer header (token dikirim ke backend)" "$output"
    else
        check_fail "client.ts TIDAK ada Authorization header — request ke backend tidak akan ter-auth!" "$output"
    fi
...
main() {
    ...
    local output_file="$OUT/BILLING-AUTH-COLLECT-$timestamp.txt"

    {
        echo "################################################################"
        echo "##  CLIENT — AUTH + BILLING/SUBSCRIPTION COVERAGE REPORT"
        echo "##  [VERCEL VIBES — Aug 2026]"
    ...
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}  AUTH + BILLING COVERAGE COLLECTOR — [Aug 2026]           ${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════${NC}"
    ...
    # ── 10. APP ROUTE — SUBSCRIPTION PAGE ────────────────────────
    section_header "10. APP ROUTE — dashboard/subscription" "$output_file"
    collect_file "$SRC_DIR/app/[locale]/(dashboard)/dashboard/subscription/page.tsx" "$output_file"

    # ── 11. API LAYER — SUBSCRIPTION ─────────────────────────────
    section_header "11. API LAYER — subscription.ts" "$output_file"
    collect_file "$SRC_DIR/lib/api/subscription.ts" "$output_file"

    # ── 12. HOOK — SUBSCRIPTION PLAN ─────────────────────────────
    section_header "12. HOOK — use-subscription-plan.ts" "$output_file"
    collect_file "$SRC_DIR/hooks/dashboard/use-subscription-plan.ts" "$output_file"

    # ── 13. UI — UPGRADE MODAL ───────────────────────────────────
    section_header "13. UI — upgrade-modal.tsx" "$output_file"
    collect_file "$SRC_DIR/components/dashboard/shared/upgrade-modal.tsx" "$output_file"

    # ── 14. TYPES — TENANT ───────────────────────────────────────
    section_header "14. TYPES — tenant.ts (subscriptionTier, subscriptionStatus)" "$output_file"
    collect_file "$SRC_DIR/types/tenant.ts" "$output_file"

    # ── 15. CONFIG — FEATURES ────────────────────────────────────
    section_header "15. CONFIG — features.ts (flag subscription)" "$output_file"
    collect_file "$SRC_DIR/lib/config/features.ts" "$output_file"

    # ── 16. AUTH LAYER YANG DIPAKAI BILLING ──────────────────────
    section_header "16. AUTH LAYER — dipakai billing (store + hook)" "$output_file"
    collect_file "$SRC_DIR/hooks/dashboard/use-tenant.ts" "$output_file"
    collect_file "$SRC_DIR/hooks/dashboard/use-checkout.ts" "$output_file"
    ...
    echo -e "\n${CYAN}── BILLING CHECKS ──────────────────────────────────────────${NC}"
    check_subscription_has_tripay "$output_file"
    check_subscription_has_ls "$output_file"
    check_hook_handles_tripay_status "$output_file"
    check_upgrade_modal_has_tripay "$output_file"
    check_upgrade_modal_has_ls "$output_file"
    check_tenant_types_subscription "$output_file"
    check_subscription_page_no_use_client "$output_file"
    check_client_has_auth_header "$output_file"
    ...
    echo ""
    echo -e "${color}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${color}║  BILLING + AUTH COLLECT — SUMMARY                  ║${NC}"
    echo -e "${color}╠════════════════════════════════════════════════════╣${NC}"
